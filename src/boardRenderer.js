function renderBoard(player) {
            ...
            if (playerBoard.placeShip(node.x, node.y, shipLength, direction)) {
                if (direction === 'vertical') {
                    for (let i = 0; i < shipLength; i++) {
                        const cellPosition = document.querySelector(`.pos_${node.x+i}_${node.y}`);
                        cellPosition.classList.add('white');
                    }
                } else if (direction === 'horizontal') {
                    for (let i = 0; i < shipLength; i++) {
                        const cellPosition = document.querySelector(`.pos_${node.x}_${node.y+i}`);
                        cellPosition.classList.add('white');
                    }
                }
    ...
    ships.forEach((shipLength, index) => {
        ...
        shipDiv.setAttribute('id', `ship-${index}`);
